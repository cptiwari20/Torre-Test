...
export default ({ result }) => {
  ...
  const renderOrganisationImage = (organizations) => {
    if (organizations) {
      if (organizations[0].picture !== null) {
        ...
        <div className="job-card-top">
          ...
          <div className="job-details">
            <div className="job-details-main">
              ...
              <span className="fw-bold">{result.compensation && result.compensation.visible && renderCompensation(result.compensation.data)}</span>
            </div>
            <div className="job-details-extra">
              <div className="mini-card">
                ...
                {result.skills &&
                  ...
                  result.skills.map((skill, i) => {
                    return (
                      <span key={i + skill.name} className="pill">
                        {skill.name}
                      </span>
                    );
                  })}
              ...
              <div className="mini-card">
                ...
                {result.members &&
                  ...
                  result.members.map((member, i) => {
                    return (
                      <a key={i + member.username}   href={`https://bio.torre.co/en/${member.username}`} target="_blank">
              ...
              <div className="mini-card">
                ...
                <span className="question">{result.questions ? (result.questions.length ? result.questions[0].text : "No question found!") : "No question found!"}</span>
              </div>
            </div>
          </div>
        </div>
        <div className="job-card-bottom">
          <div className="job-cta">
            <div className="job-cta-left">
              <span className="time fw-bold">{renderDates(result.deadline)}</span>
            ...
            <div className="job-cta-right">
              <a href={`https://torre.co/jobs/${result.id}`} target="_blank" className="btn-cta btn-cta-main fw-bold">
                View
              </a>
            </div>
          </div>
